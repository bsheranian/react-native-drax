...
export const DraxView = ({
	...
	...props
}: PropsWithChildren<DraxViewProps>): JSX.Element => {
	...
	const parentNodeHandleRef = parent
		? parent.nodeHandleRef
		: rootNodeHandleRef;
	...
	}, [id, parentId, scrollPositionRef, registerView, unregisterView]);
	...
	const getCombinedHoverStyle = useCallback(
		({
			...
			const hoverStyles: StyleProp<
				Animated.WithAnimatedValue<ViewStyle>
			>[] = [style, dimensions, hoverStyle];
	...
	const throttledHandleGestureEvent = useMemo(
		() =>
			throttle((event: DraxGestureEvent) => {
	...
	}, [viewState, trackingStatus, children]);
	...
	const combinedStyle = useMemo(() => {
		...
		const { dragging: anyDragging, receiving: anyReceiving } =
			trackingStatus;
	...
	return (
		<LongPressGestureHandler
			...
			onGestureEvent={
				onGestureEvent as any /* Workaround incorrect typings. */
			}
			enabled={draggable}
		>
			<Animated.View
